...
def create_generalization_hierarchies(generalizations_tag: list,
...
def generalize_data(dataset: DataFrame, generalization_levels: dict,
                    all_generalizations: DataFrame):
    """
    ...
    for index, level in generalization_levels.items():
        if (level == 0):
            ...
            lookup = pd.Series(
                all_generalizations[lev_string].values,
                index=all_generalizations[ind_string]).to_dict()
            for row in to_generalize:
                for original, anonymized in lookup.items():
                    if str(original) == str(row):
                        to_generalize.replace(to_replace=str(original),
                                              inplace=True,
                                              value=str(anonymized))
            df_generalized[lev_string] = to_generalize
